...
- Manager and Worker are **rig-generic**: reusable templates in
  `packs/applepi-roles/`, imported per rig via `[rigs.imports.applepi-roles]`
  in `city.toml`. Each importing rig expands them as
  `<rig>/applepi-roles.manager` / `applepi-roles.worker` with the rig root
  as the working directory (`work_dir = "{{.RigRoot}}"`). No project name
  is hardcoded in the templates.
- `scratch-proj` is a **Phase 0 development fixture** (the one rig declared
  in `city.toml`), not the final project-routing design. Register more rigs
  with `gc rig add <path> --include packs/applepi-roles` (or add a
  `[[rigs]]` entry with the same `[rigs.imports]`).
...
# register the Phase 0 fixture project as a rig (path binding only; the
# rig declaration + applepi-roles import already live in city.toml)
gc rig add ~/Projects/scratch-proj --name scratch-proj

# register another project as a rig with the same reusable roles
gc rig add ~/Projects/stockbot --include packs/applepi-roles
...
gc sling <rig>/applepi-roles.worker "<task with REPORT_TO / OBJECTIVE / ACCEPTANCE>" --title "TITLE"
...
Phase 0B — generic project-rig binding  DONE
  ✓ reusable manager/worker templates in packs/applepi-roles
  ✓ rig imports (rigs.imports.applepi-roles) expand per rig
  ✓ work_dir = {{.RigRoot}}, no hardcoded project names
